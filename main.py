...
"""
KataBump 服务器续期脚本 - GitHub Actions 版本 (v1.1)
...
def get_env_var(name: str, default: str = "") -> str:
    ...
    if value:
        ...
        logger.debug(f"环境变量 {name}: {masked}")
    return value
...
class KataBumpRenewer:
    ...
    def check_server_status(self) -> dict:
        ...
        resp = self.session.get(self.kb_renew_url, impersonate="chrome136")
        if resp.status_code != 200:
    ...
    def renew_via_browser(self) -> bool:
        ...
        # 访问续期页面
        logger.info(f"访问续期页面: {self.kb_renew_url}")
    ...
    def _browser_login_and_renew(self) -> bool:
        ...
        try:
            ...
            if email_input and password_input and submit_btn:
                email_input.input(self.kb_email)
                time.sleep(0.5)
                password_input.input(self.kb_password)
                time.sleep(0.5)
                submit_btn.click()
                logger.info("已提交登录表单")
                time.sleep(5)
    ...
    def run(self):
        """主运行流程"""
        logger.info("=" * 50)
        logger.info("KataBump 续期脚本启动 (v1.1)")
        logger.info("=" * 50)
        ...
        if missing:
            ...
            logger.error("  - KB_EMAIL: KataBump 账号邮箱")
            logger.error("  - KB_PASSWORD: KataBump 账号密码")
            logger.error("  - KB_RENEW_URL: 续期页面 URL")
